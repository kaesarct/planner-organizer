...
export async function renderAdmin() {
          ...
          (t) => `
            ...
            t.description
              ...
              : ""
                    ...
                    <td><small class="text-muted">${
                      t.assigned_to
                        ? usersMap[t.assigned_to] || "Utente"
                        : "Non assegnato"
                    }</small></td>
                    <td><span class="badge bg-${
                      t.visible ? "success" : "danger"
                    }">${t.visible ? "👁️ Visibile" : "🙈 Nascosto"}</span></td>
                    <td>
                        <button class="btn btn-${
                          t.visible ? "secondary" : "success"
                        } btn-sm" onclick="toggleTaskVisibility('${
            t.id
          }', ${!t.visible})">${t.visible ? "🙈" : "👁️"}</button>
                        <button class="btn btn-danger btn-sm" onclick="deleteTaskAdmin('${
                          t.id
                        }')">🗑️</button>
                    </td>
                </tr>
  ...
  return `
        ...
        <ul class="nav nav-tabs mb-3">
            ...
            <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#admin-polls" onclick="setTimeout(loadPolls, 100)">📊 Sondaggi</a></li>
        ...
        <div class="tab-content">
                        ...
                        <tbody>
                            ${users
                              .map(
                                ...
                                <tr>
                                    <td>
                                        <button class="btn btn-link p-0 text-decoration-none" onclick="openEditUserModal('${u.id}')">
                                            ${(u.name && u.name !== "Utente") ? u.name : (u.email ? u.email.split("@")[0] : "Utente")} ✏️
                                        </button>
                                    </td>
                                    <td>${u.email}</td>
                                    <td><span class="badge bg-${
                                    ...
                                    <td>
                                        <select class="form-select form-select-sm" style="width:auto; display:inline-block" onchange="changeUserRole('${
                                          u.id
                                        }', this.value)">
                                            <option value="base" ${
                                              u.role === "base"
                            ...
                            ${events
                              .map(
                                ...
                                <tr>
                                    ...
                                    <td><small>${new Date(
                                      e.start_date
                                    ).toLocaleDateString("it-IT")}</small></td>
                                    <td><small class="text-muted">${
                                      e.location || "-"
                                    }</small></td>
                                    <td><button class="btn btn-danger btn-sm" onclick="deleteEventAdmin('${
                                      e.id
                                    }')">🗑️</button></td>
                                </tr>
                            `
            ...
            <div class="tab-pane fade" id="admin-polls">
                <h5>Gestione Sondaggi</h5>
                <div class="table-responsive">
                    <table class="table table-striped">
                        <thead><tr><th>Domanda</th><th>Opzioni</th><th>Scadenza</th><th>Voti</th><th>Stato</th><th>Azioni</th></tr></thead>
                        <tbody id="admin-polls-tbody"></tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="admin-settings">
                ...
                <div class="card mb-3">
                    <div class="card-header"><strong>Permessi Sondaggi</strong></div>
                    <div class="card-body">
                        <table class="table table-bordered table-sm">
                            <thead><tr><th>Azione</th><th class="text-center">Base</th><th class="text-center">Segretario</th><th class="text-center">Reviewer</th><th class="text-center">Admin</th></tr></thead>
                            <tbody id="poll-permissions-tbody"></tbody>
                        </table>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Modifica Nome Utente</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="saveUserName()">Salva</button>
                    </div>
                </div>
            </div>
        </div>
...
window.loadPermissions = async function () {
  try {
    ...
    for (const role of ["base", "segretario", "reviewer", "admin"]) {
      ...
      if (!permDoc.exists()) {
        const defaultPerms = {
          event_create: role !== "base",
          event_edit: role !== "base",
          event_delete: role !== "base",
          event_field_title: role !== "base",
          event_field_description: role !== "base",
          event_field_type: role !== "base",
          event_field_location: role !== "base",
          event_field_start_date: role !== "base",
          event_field_end_date: role !== "base",
          task_create: role !== "base",
          task_edit: true,
          task_delete: role !== "base",
          task_field_title: true,
          task_field_description: true,
          task_field_status: true,
          task_field_priority: role !== "base",
          task_field_due_date: role !== "base",
          task_field_assigned: role !== "base",
          poll_create: role !== "base",
          poll_vote: true,
          poll_delete: role === "admin",
          ...
          (perm) => `
                <tr>
                    ...
                    <td class="text-center"><input type="checkbox" class="form-check-input" ${
                      permissions.base[perm.key] ? "checked" : ""
                    } onchange="togglePermission('base', '${
            perm.key
          }', this.checked)"></td>
                    <td class="text-center"><input type="checkbox" class="form-check-input" ${
                      permissions.reviewer[perm.key] ? "checked" : ""
                    } onchange="togglePermission('reviewer', '${
            perm.key
          }', this.checked)"></td>
                    <td class="text-center"><input type="checkbox" class="form-check-input" ${
                      permissions.admin[perm.key] ? "checked" : ""
                    } disabled></td>
                </tr>
            `
        )
        ...
        .map(
          (perm) => `
            ...
            perm.key
                    ...
                    <td class="text-center"><input type="checkbox" class="form-check-input" ${
                      permissions_segretario[perm.key] ? "checked" : ""
                    } onchange="togglePermission('segretario', '${
            perm.key
          }', this.checked)"></td>
                    <td class="text-center"><input type="checkbox" class="form-check-input" ${
                      permissions.reviewer[perm.key] ? "checked" : ""
    ...
    renderTableWithSegretario("poll-permissions-tbody", [
      { key: "poll_create", label: "Crea Sondaggi" },
      { key: "poll_vote", label: "Vota Sondaggi" },
      { key: "poll_delete", label: "Elimina Sondaggi" },
    ]);
  } catch (error) {
  ...
  if (tbody) {
    tbody.innerHTML = reports
      .map(
        (r) => `
            <tr>
                <td><strong>${new Date(r.meeting_date).toLocaleDateString(
                  "it-IT",
                  {
                    weekday: "long",
                    day: "2-digit",
                ...
                <td><small class="text-muted">${r.content.substring(0, 100)}${
          r.content.length > 100 ? "..." : ""
        }</small></td>
                <td><button class="btn btn-danger btn-sm" onclick="deleteReport('${
                  r.id
                }')">🗑️</button></td>
            </tr>
        `
...
window.loadPolls = async () => {
  ...
  polls.sort(
    (a, b) =>
      new Date(b.created_at?.seconds * 1000 || 0) -
      new Date(a.created_at?.seconds * 1000 || 0)
  );

  const tbody = document.getElementById("admin-polls-tbody");
  if (tbody) {
    tbody.innerHTML = polls
      .map((p) => {
        const now = new Date();
        ...
        return `
                <tr>
                    ...
                    <td><small>${deadline.toLocaleDateString("it-IT", {
                      day: "2-digit",
                      month: "2-digit",
                      year: "numeric",
                      hour: "2-digit",
                    ...
                    <td><small class="text-muted">${totalVotes} voti</small></td>
                    <td><span class="badge bg-${
                      isExpired ? "secondary" : "success"
                    }">${isExpired ? "Scaduto" : "Attivo"}</span></td>
                    <td>
                        <button class="btn btn-primary btn-sm" onclick="editPollDeadline('${
                          p.id
                        }', '${p.deadline}')">📅 Modifica Scadenza</button>
                        <button class="btn btn-danger btn-sm" onclick="deletePoll('${
                          p.id
                        }')">🗑️</button>
                    </td>
                </tr>
...
window.editPollDeadline = async (pollId, currentDeadline) => {
  const newDeadline = prompt(
    "Inserisci la nuova data di scadenza (formato: YYYY-MM-DDTHH:MM):",
    currentDeadline.slice(0, 16)
  );
  ...
  await updateDoc(doc(db, "polls", pollId), { deadline: newDeadline });
  loadPolls();
};
...
window.openEditUserModal = async (userId) => {
    ...
    const currentName = (userData.name && userData.name !== 'Utente') ? userData.name : '';
